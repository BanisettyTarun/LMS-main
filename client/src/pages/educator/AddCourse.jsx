...
function AddCourse() {
  ...
  const handleSubmit = async (e)=> {
    ...
    try {
      ...
      const res = await axios.post('/api/educator/add-course', formData, {
      ...
      if (res.data && res.data.success) {
        ...
        alert('Failed to add course: ' + (res.data?.message || 'Unknown error'));
      }
    } catch (err) {
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0 relative'>
      {isLoading && (
        <div className="absolute inset-0 bg-white/80 flex items-center justify-center z-50">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
            <p className="text-gray-600 font-medium">Adding your course...</p>
            <p className="text-gray-500 text-sm mt-2">Please wait while we upload your course</p>
          </div>
        </div>
      )}
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          ...
          <div className='flex md:flex-row flex-col items-center gap-3'>
            ...
            <label htmlFor="thumbnailImage" className='flex items-center gap-3'>
              <img src={assets.file_upload_icon} alt="" className='p-3 cursor-pointer bg-blue-500 rounded' />
              <input type="file" id='thumbnailImage' onChange={e => setImage(e.target.files[0])} accept='image/*' hidden />
              {image ? <img className='max-h-10' src={URL.createObjectURL(image)} alt="" /> : null}
            </label>
          </div>
        </div>
        <div className='flex flex-col gap-1'>
                ...
                <div className='p-4'>
                  {chapter.chapterContent.map((lecture, lectureIndex) => (
                    <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                      ...
                      <img src={assets.cross_icon} onClick={()=> handleLecture('remove', chapter.chapterId, lectureIndex)} className='cursor-pointer' alt="" />
              ...
              <div className='fixed inset-0 flex items-center justify-center bg-gray-800/50'>
                <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                  ...
                  <div className='mb-2'>
                    ...
                    <input 
                      ...
                      className={`mt-1 block w-full border rounded py-1 px-2 ${
                        lectureDetails.lectureUrl && !isValidYouTubeUrl(lectureDetails.lectureUrl) 
                          ? 'border-red-500' 
                          : ''
                      }`}
                  ...
                  <div className='my-4 gap-2 flex'>
                    <p>Is Preview Free</p>
                    <input type="checkbox" className='mt-1 scale-x-125'
        ...
        <button 
          type='submit' 
          disabled={isLoading}
          className={`w-max py-2.5 px-8 rounded my-4 flex items-center gap-2 ${
            isLoading 
